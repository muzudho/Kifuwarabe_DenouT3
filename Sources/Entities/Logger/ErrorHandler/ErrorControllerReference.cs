...
namespace Grayscale.Kifuwarakaku.Entities.Logger
{
    ...
    public static class ErrorControllerReference // partial /// partial … ロガー定数を拡張できるクラスとして開放。
    {
        ...
        public static Dictionary<ILogTag, ILogRecord> LogMap
        {
            get
            {
                if (ErrorControllerReference.logMap == null)
                {
                    ErrorControllerReference.logMap = new Dictionary<ILogTag, ILogRecord>();
                }
                return ErrorControllerReference.logMap;
            }
        }
        ...
        {
            ErrorControllerReference.LogMap.Add(key, value);
        }
