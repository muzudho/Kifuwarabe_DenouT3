...
namespace Grayscale.P531Hyokakansu.L499UtilFv
{
    ...
    public abstract class Util_FvParamIndex
    {
        ...
        public static int ParamIndex_Banjo(RO_Star p_koma)
        {
            ...
            {
                UtilOwataMinister.ERROR.DonimoNaranAkirameta("二駒関係の先後不明の駒");
            }
            else if (index_komasyurui == -1)
            {
                UtilOwataMinister.ERROR.DonimoNaranAkirameta("二駒関係の駒種類が対象外の駒");
            }
        ...
        public static int ParamIndex_Moti(SkyConst src_Sky, Playerside pside, Komasyurui14 syurui)
        {
            ...
            if (index_playerside == -1)
            {
                UtilOwataMinister.ERROR.DonimoNaranAkirameta("二駒関係の持ち駒_先後不明の駒");
            }
            else if (index_komasyurui == -1)
            {
                UtilOwataMinister.ERROR.DonimoNaranAkirameta("二駒関係の持ち駒_駒種類が対象外の駒");
            }
